...
Tipos numéricos baseados em bits, incluindo conceitos relacionados a ponto flutuante.
...
pai(joao, pedro). (João é pai de Maria. João é pai de Pedro.)
...
irmãos(X, Y) :- pai(Z, X), pai(Z, Y), X \\= Y. (X e Y são irmãos se existe um Z que é pai de X e também pai de Y, e X é diferente de Y.)
...
?- irmãos(maria, pedro). Resposta do sistema: true. O motor de inferência encontra que joao é pai de ambos e que maria ≠ pedro, satisfazendo a regra.
